...
const Board = () => {
  ...
  const { randomBalls, setRandomBalls, completed } = useContext(ballsContext);
  const initialRandomBall = [...randomBalls];
  const config = {
    angle: 90,
    spread: 360,
    startVelocity: 40,
  ...
  return (
    <div className='board-container'>
      <header className='header'>
        ...
        <Link to='/board' onClick={setRandomBalls(initialRandomBall)} ><button className='header-controls'>Reastart</button></Link>

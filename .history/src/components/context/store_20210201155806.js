...
const BallsContextProvider = (props) => {
  ...
  const chooseBallHandler = (index) => {
    ...
    setMovedBall(Object.assign(tempRandomBalls[tube][tempRandomBalls[index].length - 1], { index }));
  ...
  const moveBallHandler = (tube) => {
    if (movedBall === null) {
      ...
      if (tube === movedBall.tube) {
        rmChosenBallHandler(tube);
      } else {
        if (movedBall.num === randomBalls[tube][randomBalls[tube].length - 1].num) {
          if (randomBalls[tube].length < 4) {
            let tempRandomBalls = [...randomBalls];
            tempRandomBalls[tube].push(tempRandomBalls[movedBall.tube][tempRandomBalls[movedBall.tube].length - 1]);
            tempRandomBalls[movedBall.tube].splice([tempRandomBalls[movedBall.tube].length - 1], 1);
            setRandomBalls(tempRandomBalls);
            setMovedBall(null);
          } else {
            rmChosenBallHandler(tube);

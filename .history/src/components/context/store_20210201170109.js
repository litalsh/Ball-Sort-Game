...
const BallsContextProvider = (props) => {
  ...
  const moveBallHandler = (tube) => {
    if (randomBalls[tube].length < 1 && movedBall === null) {
      return
    }
    if (movedBall === null) {
      ...
      if (tube === movedBall.index) {
        ...
        if (randomBalls[tube].length === 4) {
          ...
          if (movedBall.num === randomBalls[tube][randomBalls[tube].length - 1].num || randomBalls[tube].length < 4) {

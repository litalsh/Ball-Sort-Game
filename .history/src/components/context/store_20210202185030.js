...
const BallsContextProvider = (props) => {
  ...
  const undoHandler = () => {
    if (undoCount === 0){
      ...
      let tempRandomBalls = [...randomBalls];
      let undoItem = tempRandomBalls.splice(tempRandomBalls[undoMoves[undoMoves.length-1].new].length-1, 1);
      tempRandomBalls[undoMoves[undoMoves.length-1].old].push(undoItem);
    }
  }

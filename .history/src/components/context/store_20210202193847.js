...
const BallsContextProvider = (props) => {
  ...
  const undoHandler = () => {
    if (undoCount === 0){
      ...
      let tempRandomBalls = [...randomBalls];
      let temp = tempRandomBalls[undoMoves[undoMoves.length-1].newIndex];
      console.log(temp,'undoMoves', undoMoves)
      let undoItem = tempRandomBalls.splice(tempRandomBalls[undoMoves[undoMoves.length-1].newIndex][temp.length-1], 1);

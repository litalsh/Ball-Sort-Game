...
const BallsContextProvider = (props) => {
  ...
  const [initialRandomBall, setInitialRandomBall] = useState();
  ...
  const shuffle = (array) => {
    ...
    setInitialRandomBall(shuffled.concat(emptyTubes));
  }
  ...
  const undoHandler = () => {
    if (undoCount === 0){
      ...
      let tempRandomBalls = [...randomBalls];
      let previousTube = tempRandomBalls[undoMoves[undoMoves.length-1].oldIndex];
      console.log('prevTube', previousTube, 'undoMoves', undoMoves)
      let undoItem = tempRandomBalls.splice(tempRandomBalls[undoMoves[undoMoves.length-1].newIndex][tempRandomBalls[undoMoves[undoMoves.length-1].newIndex].length-1], 1);

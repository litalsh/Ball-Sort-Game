...
const BallsContextProvider = (props) => {
  ...
  const changeTubeHandler = (index) => {
    ...
    console.log('temp', tempRandomBalls, 'index', index);
    setRandomBalls(tempRandomBalls);
    setMovedBall(null);
  }
  ...
  const moveBallHandler = (tube) => {
    ...
    if (movedBall !== null && tube === movedBall.index) {

      rmChosenBallHandler(tube);
    };

    if (movedBall !== null && randomBalls[tube].length === 4) {
      rmChosenBallHandler(tube);
    };

    if (movedBall !== null && randomBalls[tube].length < 1) {
